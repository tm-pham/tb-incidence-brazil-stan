...
# Provisional coded values standing in for the dictionary: 1 = new, 2 = relapse,
# 3 = re-entry after default, 4 = transfer, 5 = post-mortem.
KEEP <- c("1", "2")

test_that("summarise_notifications keeps only the requested entry types", {
  sinan <- data.table(
    entry_type = c("1", "2", "3", "4", "5", "1"),
    muni_res   = rep("355030", 6),
    muni_occ   = rep("355030", 6),
    year       = rep(2018L, 6)
  )
  out <- summarise_notifications(sinan, keep_entry = KEEP)
  # New (x2) + relapse (x1) counted; re-entry, transfer, post-mortem dropped.
